...
y_meas = min(x_true(3,:)+ sqrt(R) * random_measurement,0.98);
...
y_pred_plot(1) = x0_true(3);
y_pred_plot2(1) = x0_true(3);
...
%     x_k_k(3) = V_consistent;
    ...
    y_pred_plot(k) = x_pred_plot(3,k+1);
    % End of Prediction step

    % Begin Measurements update

    % --- Update Step (Discrete-time measurement update) ---
    
    % Get the current measurement
    y_kplus1 = y_meas(k+1);
    % Calculate Innovation 
    y_innov = y_kplus1 - min(x_kplus1_k(3),0.98);
    % Calculate Jacobian-Like of the Measurement function
    C_kplus1 = SLmin([x_kplus1_k(3),Sx3_k],[0.98,0*Sx3_k]);
    if C_kplus1 == [0,0,0]
        L_kplus1 = [0;0;0];
    else  
...
y_pred_plot(k+1) = x_pred_plot(3,k+1);
%% Plot Results
close all; clc
font_size = 50;
...
plot(t_meas, x_pred_plot(3,:), '-','color','#77AC30', 'LineWidth', 3); hold on;
yline(0.98, '--','color','k', 'LineWidth', 4); 
legend('True', 'Estimated','Predicted','Max reading', 'Location', 'best','Fontsize',30);
...
font_size = 50;
fig = figure('DefaultAxesFontSize',font_size,'defaultLineLineWidth',7,'DefaultAxesTitleFontWeight','bold');
plot(t_meas, x_true(3,:), 'd', 'color', '#80B3FF',  'LineWidth', 3, 'MarkerSize', 10); hold on;
plot(t_meas, y_meas, 'r*', 'MarkerSize', 2); hold on;
plot(t_meas, x_est(3,:), 'k-*', 'LineWidth', 3); hold on;
plot(t_meas, y_pred_plot, '-','color','#77AC30', 'LineWidth', 4); 
...
function [Diff_States_lno,Diff_States_lo,Alg_States_lno,Alg_States_lo] = LSERC(z0,t0,tf,N)
    ...
    for ti=1:StepCount
        ...
        LD_Y_theta(step,:) = SLmin([Z(step,3),Sw],[0.98,0*Sw] );
        step = step+1;
    end
    M = [[1 0]',eye(2)];
